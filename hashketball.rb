...
def find_player name
  sought_player = allPlayers.find { |player| player[:player_name] == name }
end
...
def find_team team_name
  if game_hash[:home][:team_name].include?(team_name)
    ...
    "This team is not being tracked."
  end
end
...
def big_shoe_rebounds
  allPlayers.max{ |player1, player2| player1[:shoe] <=> player2[:shoe] }[:rebounds]
end
...
def long_name_steals_a_ton
  player_with_longest_name == allPlayers.max_by { |player| player[:steals] }[:player_name]
end
